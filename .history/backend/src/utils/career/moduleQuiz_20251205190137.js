...
function extractGiftNumeric(q) {
  // Case 1: Parsed as { answer: { number: 5 } }
  if (q.answer && typeof q.answer.number === "number") {
    return q.answer.number;
  }

  // Case 2: Parsed as { answer: { text: "#5" } }
  if (q.answer && typeof q.answer.text === "string") {
    const match = q.answer.text.match(/#\s*([-]?\d+(\.\d+)?)/);
  ...
  try {
    ...
    if (Array.isArray(q.choices) && q.choices.length > 0) {
      for (const choice of q.choices) {
        answerNumber = extractGiftNumeric(choice);
    ...
    if (answerNumber === null) {
      ...
      answerNumber = extractGiftNumeric(q.answer)
        ?? extractGiftNumeric(q.number)
        ?? extractGiftNumeric(q.value)
        ?? extractGiftNumeric(q.min);
    }
    ...
    if (answerNumber === null) {
      for (const key in q) {
        if (q.hasOwnProperty(key) && key !== 'stem' && key !== 'type') {
          answerNumber = extractGiftNumeric(q[key]);
          if (answerNumber !== null) break;
        }
      }
    }
