...
function getNumericAnswer(q) {
  // 1) Standard: q.answer.number
  if (q.answer && typeof q.answer.number === "number") {
    return q.answer.number;
  }

  // 2) Sometimes gift-pegjs stores q.number
  ...
  if (typeof q.answer === "string") {
    const m = q.answer.match(/#\s*(-?\d+(\.\d+)?)/);
    if (m) return Number(m[1]);
  }

  // 4) Sometimes inside q.answer.text
  if (q.answer && typeof q.answer.text === "string") {
    const m = q.answer.text.match(/#\s*(-?\d+(\.\d+)?)/);
    if (m) return Number(m[1]);
  }
...
async function handleNumerical(q, importId) {
  try {
    ...
    // extract answer straight from raw GIFT text
    const raw = q.raw ?? "";          // <--- THIS IS THE key
    ...
    const answerNumber = Number(match[1]);
