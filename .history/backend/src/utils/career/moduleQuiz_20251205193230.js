...
async function handleNumerical(q, importId) {
  try {
    ...
    // gift-pegjs ALWAYS gives numerical answers in choices[0].number
    let numeric = null;

    if (Array.isArray(q.choices) && q.choices.length > 0) {
      const c = q.choices[0];

      // Case 1: number is directly here
      if (typeof c.number === "number") {
        numeric = c.number;
      }

      // Case 2: nested: { number: { number: 5 } }
      else if (c.number && typeof c.number.number === "number") {
        numeric = c.number.number;
      }
    }
    ...
    if (numeric === null) {
      return {
        ok: false,
        reason: "Numeric answer not found",
        raw: q
      };
    }
