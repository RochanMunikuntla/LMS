...
import { Quiz } from "../models/lmsQuiz.js";
import { Question } from "../models/lmsQuiz.js";
import Announcement from "../models/announcement.js";
import Material from "../models/materials.js";
import Course from "../models/course.js";
import createQuiz from "../../../utils/career/moduleQuiz.js"
...
export const createQuiz = async (req, res) => {
    try {
        ...
        const { importId } = await createQuiz(filePath);

        const { courseId } = req.params;
        const { title, shuffleQuestions } = req.body;

        const questions = await Question.find({ importId: importId }).sort({ seq: 1 }).select('_id').lean();
        ...
        res.status(201).json({ message: "Quiz created successfully!", questions });
    } catch (err) {
...
export const deleteQuiz = async (req, res) => {
    try {
        ...
        await Question.deleteMany({ importId });
