...
export const getQuiz = async (req, res) => {
    try {
        ...
        const quizAttempts = await lmsQuiz.findById(quizId).select("attempts");

        if (attemptsCount >= quizAttempts.attempts) {
            return res.status(403).json({
                message: "Maximum number of attempts reached"
            });
        }

        const quizDue = await lmsQuiz.findById(quizId).select("dueAt maxAttempts");

        if (quizDue.dueAt && new Date() > quizDUe.dueAt) {
            return res.status(403).json({
        ...
        const quiz = await lmsQuiz.findById(quizId).select("title attempts").populate({
            path: "course",
...
export const reviewQuiz = async (req, res) => {
    try {
        const studentId = req.session?.userId || req.user?.id;
        const { quizId, reviewId } = req.params;
