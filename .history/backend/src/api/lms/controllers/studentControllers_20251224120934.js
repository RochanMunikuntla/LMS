...
export const getQuiz = async (req, res) => {
    try {
        ...
        const quizDoc = await lmsQuiz.findById(quizId).select("attempts dueAt disabled");

        if (attemptsCount >= quizDoc.attempts) {
            quizDoc.disabled = true;
        }

        if (quizDoc.dueAt && new Date() > quizDoc.dueAt) {
            quizDoc.disabled = true;
        }
        ...
        res.status(201).json(quiz, quizDoc.disabled);
    } catch (err) {
