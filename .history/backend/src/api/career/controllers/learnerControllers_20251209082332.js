...
export const createProjectCollab = async (req, res) => {
    try {
        ...
        return res.status(201).json({ message: "Project collaboration created successfully", project });
    } catch (error) {
        return res.status(500).json({ message: "Server error", error: error.message });
    }
}
...
export const getAllProjectCollab = async (req, res) => {
    try {
        ...
        const projectCollabs = await ProjectCollab.find({student: { $ne: studentId }
        }).sort({ createdAt: -1 });
