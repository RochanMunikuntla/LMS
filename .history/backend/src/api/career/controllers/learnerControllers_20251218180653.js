...
import { runBatchSubmissions } from "../services/runBatchSubmissions.js";

export const testBatch = async (req, res) => {
  try {
    const submissions = req.body.submissions; // expect an array
    
    if (!Array.isArray(submissions)) {
      return res.status(400).json({ message: "submissions must be an array" });
    }

    const result = await runBatchSubmissions(submissions);
