...
export const filterProjectCollab = async (req, res) => {
    try {
        const tagsInput = req.body.tags ?? req.query.tags; // support both
let tagsArray = [];

// normalize possible query formats
if (typeof tagsInput === 'string') {
  // comma separated "react,node"
  tagsArray = tagsInput.split(',').map(s => s.trim()).filter(Boolean);
} else if (Array.isArray(tagsInput)) {
  tagsArray = tagsInput.map(s => String(s).trim()).filter(Boolean);
}

// optional: toLowerCase normalization
