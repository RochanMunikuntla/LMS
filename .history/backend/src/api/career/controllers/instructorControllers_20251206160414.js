    ...
    try {
        ...
        if (!seedResult || !seedResult.ok) {
            return res.status(500).json({
                ...
                reason: seedResult?.reason ?? "Unknown"
            });
        }

        const importId = seedResult.importId;

        // fetch inserted question ids
        const questions = await careerQuestion
            .find({ importId })
            .sort({ seq: 1 })   // if you rely on seq; otherwise remove
            .select("_id")
            .lean();
        ...
        // update module to reference this quiz (if required)
