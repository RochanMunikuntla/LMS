...
export const submitCodeToTopic = async (req, res) => {
    try {
        ...
        console.error("submitCodeToProblem:", err);
        return res.status(500).json({
            message: "Server error",
            error: err.message
        });
...
export const getProblem = async (req, res) => {
    try {
        ...
        const problem = await CodingQuestion.findOne({ problemId }).select("-_id problemId title difficulty");
        
        const tc = await CodingQuestion.findById(problemId).select("testCases");
...
export const submitCodeToProblem = async (req, res) => {
    try {
        ...
        const idealComplexity = await CodingQuestion.findById(problemId).select("-_id idealComplexity");

        const hiddenTC = await CodingQuestion.findById(problemId).select("-_id testCases");
        ...
        const userMaxTimeComplexity = Math.max(
            ...results.map(r => Number(r.time || 0))
        );
        const userMinTimeComplexity = Math.min(
            ...results.map(r => Number(r.time || 0))
        );
        const userMaxSpaceComplexity = Math.max(
        ...
        res.status(201).json({
            ...
            userMaxTimeComplexity: verdict == "Accepted" ? userMaxTimeComplexity : null,
            userMinTimeComplexity: verdict == "Accepted" ? userMinTimeComplexity : null,
            userMaxSpaceComplexity: verdict == "Accepted" ? userMaxSpaceComplexity : null
        });
