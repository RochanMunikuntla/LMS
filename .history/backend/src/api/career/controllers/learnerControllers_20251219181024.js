...
export const submitCodeToTopic = async (req, res) => {
    try {
        ...
        const testcases = await TestCase.find({ topic: topicId }).select("input output");
        ...
        for (let i = 0; i < result.length; i++) {
            ...
            if (normalize(r.stdout) !== normalize(testcases[i].expectedOutput)) {
