...
export const runCodeForTopic = async (req, res) => {
    try {
        ...
        const input = await Topic.findById(topicId).select("testCases").populate({
            path: "testCases",
            match: { visible: true },
            select: "-_id input"
        })

        const stdins = input.testCases.map(tc => tc.input);

        const result = await batchRunCode(language_id, source_code, stdin);

        return res.status(200).json({ stdins });
    ...
    try {
        ...
        if (!languageId || !sourceCode) {
            return res.status(400).json({
                message: "languageId and sourceCode are required"
            });
        }

        const testcases = await TestCase.find({ topic: topicId }).select("input");
        ...
        const result = await batchRunCode(submissions);
