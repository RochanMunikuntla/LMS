...
export const addTagToProjectCollab = async (req, res) => {
    try {
        ...
        const userId = req.session?.userId || req.user?.id;
        const { tags } = req.body;
        if (!tags) return res.status(400).json({ message: "tag is required" });
        ...
        await ProjectCollab.updateOne(
            { _id: projecthubId },
            { $addToSet: { tags: { $each: existingTagIds } } }
        );


        // --- 4. Update Tag Counts ---
        // Use updateMany to increment the count for ALL tags that were successfully added/found
