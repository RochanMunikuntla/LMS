...
export const createQuiz = async (req, res) => {
    ...
    try {
        ...
        console.log("DEBUG: file received:", filePath);

        // Await the seeder and destructure the returned object properly.
        // seedQuiz returns { ok, importId, total, insertedCount, errors }.
        const seedResult = await seedQuiz(filePath);

        const { importId } = seedResult; // valid destructure of returned object
        console.log("DEBUG: seed returned importId:", importId);

        // Fetch the inserted question IDs (use lean for performance)
        ...
        // update module to reference quiz (if desired)
        if (moduleId) {
