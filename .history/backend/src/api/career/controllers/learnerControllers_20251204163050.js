...
export const profile = async (req, res) => {
    try {
        ...
        const enrolledRoadmapsProgress = enrolledRoadmaps.roadmaps.map(r => ({
            title: r.title,
            moduleCount: r.modules.length,
            topicCount: r.modules.reduce((sum, mod) => sum + mod.topics.length, 0)
        })) 

        const totalCount = enrolledRoadmaps.roadmaps.map()
        ...
        res.status(200).json({ enrolledRoadmaps, countRoadmaps });
    } catch (error) {
        return res.status(500).json({ message: "Server error", error: error.message });
    }
}
