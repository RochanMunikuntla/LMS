...
export const getProjectCollab = async (req, res) => {
    try {
        const { const { projectCollabId } = req.params;

    if (!projectCollabId) {
      return res.status(400).json({ message: "projectCollabId is required" });
    }

    const projectCollab = await ProjectCollab.findById(projectCollabId);

    if (!projectCollab) {
      return res.status(404).json({ message: "Collaboration not found" });
    }

    return res.status(200).json({ projectCollab }); } = req.params;
