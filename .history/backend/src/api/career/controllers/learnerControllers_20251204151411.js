...
export const profile = async (req, res) => {
    try {
        const student = req.session?.userId || req.user?.id;

        const enrolledRoadmaps = await Student.findById(student).select("-_id -__t roadmaps").populate({
            path: "roadmaps",
            select: "_id"
        });
        
        const countRoadmaps = enrolledRoadmaps.roadmaps.length;

        const completedTopics = await RoadmapProgress.countDocuments({
            student: student,
            completed: true
        });

        const totalTopicsFromRoadmap = await RoadmapProgress.countDocuments({
            student: student,
            roadmap: enrolledRoadmaps
        })

        const topicProgress = completedTopics;




        res.status(200).json({ enrolledRoadmaps, countRoadmaps, topicProgress, totalTopicsFromRoadmap });
    } catch (error) {
        return res.status(500).json({ message: "Server error", error: error.message });
    }
}
