...
export const editProjectCollab = async (req, res) => {
    try {
        const projecthubId = req.params.projecthudId;
        const {title, description, tags} = req.body;
        ...
        if(title) projectCollab.title = title;
        if(description) projectCollab.description = description;
        if(tags) projectCollab.tags.push(tags);
