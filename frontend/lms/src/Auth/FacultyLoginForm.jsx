...
export default function FacultyLoginForm({ onSuccessRedirect } = {}) {
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({})
    try {
      const payload = { facultyId: formData.facultyId, password: formData.password }
      await client.post("/faculty/login", payload)
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-900 px-4 text-gray-100">
      <div className="w-full max-w-md bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-700">
        ...
        <div className="mb-6 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">Access your faculty dashboard</p>
        </div>
          ...
          <div>
            ...
            <div className="mt-1 relative">
              ...
              <input
                ...
                disabled={isLoading}
                className={`block w-full pl-10 pr-3 py-2 rounded-lg bg-gray-900 border ${errors.facultyId ? 'border-red-500' : 'border-gray-700'} text-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
              />
            </div>
            {errors.facultyId && <p className="mt-2 text-sm text-red-400">{errors.facultyId}</p>}
          </div>
