...
// Dark Mode Version — full Tailwind dark theme

export default function AdminLoginForm({ onSuccessRedirect } = {}) {
    ...
    try {
      ...
      if (typeof onSuccessRedirect === "function") {
        try {
          ...
          console.error("onSuccessRedirect threw an error:", err)
        }
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-900 px-4 text-gray-100">
      <div className="w-full max-w-md bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-700">
        ...
        <div className="mb-6 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">Access admin dashboard and settings</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Admin ID */}
          <div>
            <label htmlFor="adminId" className="block text-sm font-medium text-gray-300">Admin ID</label>
            <div className="mt-1 relative">
              ...
              <input
                ...
                disabled={isLoading}
                className={`block w-full pl-10 pr-3 py-2 rounded-lg bg-gray-900 border ${errors.adminId ? 'border-red-500' : 'border-gray-700'} text-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
              />
            </div>
            {errors.adminId && <p className="mt-2 text-sm text-red-400">{errors.adminId}</p>}
          </div>
