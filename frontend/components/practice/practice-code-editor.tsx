...
export function PracticeCodeEditor({ starterCode }: PracticeCodeEditorProps) {
      ...
      <div className="flex items-center justify-between border-b border-[#333] bg-[#1e1e1e] px-4 py-2">
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded bg-[#333] px-3 py-1.5 text-sm text-gray-200 hover:bg-[#404040]"
          >
            {languageOptions.find((l) => l.value === language)?.label}
            <ChevronDown className="h-3 w-3" />
          </button>
          {isDropdownOpen && (
            <div className="absolute left-0 top-full z-10 mt-1 rounded border border-[#444] bg-[#2d2d2d] py-1 shadow-lg">
              {languageOptions.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => handleLanguageChange(opt.value as "python" | "javascript" | "cpp" | "java")}
                  className={`block w-full px-4 py-1.5 text-left text-sm hover:bg-[#404040] ${
